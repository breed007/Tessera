...
var table = map[int]Risk{
	...
	2049:  {"high", "file-sharing", "NFS exposed", "NFS export — often host-trust only, no user auth"},
	3389:  {"high", "remote-access", "RDP exposed", "Remote Desktop — frequent brute-force / exploit target"},
	5984:  {"high", "database", "CouchDB exposed", "CouchDB — ships with an open 'admin party' by default"},
	2379:  {"high", "database", "etcd exposed", "etcd cluster store — frequently unauthenticated; holds secrets"},
	10250: {"high", "orchestration", "Kubelet API exposed", "Kubelet — anonymous auth can mean node-level RCE"},

	139:   {"medium", "file-sharing", "NetBIOS exposed", "Legacy Windows file/printer sharing"},
	548:   {"medium", "file-sharing", "AFP exposed", "Apple Filing Protocol — legacy file sharing"},
	111:   {"medium", "file-sharing", "rpcbind exposed", "Portmapper — enumerates NFS/RPC; UDP amplification risk"},
	161:   {"medium", "management", "SNMP exposed", "SNMP — often left on a default community string"},
	1900:  {"medium", "management", "UPnP/SSDP exposed", "UPnP control — device control + UDP amplification risk"},
	10000: {"medium", "admin", "Webmin exposed", "Webmin server-administration UI reachable"},
	...
	8086:  {"medium", "database", "InfluxDB exposed", "InfluxDB time-series database reachable"},
	25:    {"medium", "mail", "SMTP exposed", "SMTP — verify this is not an open relay"},
	110:   {"medium", "mail", "POP3 exposed", "POP3 — mailbox access, credentials in clear text"},
	143:   {"medium", "mail", "IMAP exposed", "IMAP — mailbox access, credentials in clear text"},
	...
	69:    {"medium", "plaintext", "TFTP exposed", "TFTP — no authentication, plaintext file transfer"},
	...
	5985:  {"medium", "remote-access", "WinRM exposed", "Windows Remote Management — remote command execution"},
	9100:  {"medium", "printing", "Raw printing (JetDirect)", "Port 9100 raw print — often unauthenticated, abusable"},
	1883:  {"medium", "iot", "MQTT exposed", "MQTT broker — frequently runs with no authentication"},
	5060:  {"medium", "voip", "SIP exposed", "SIP/VoIP — toll-fraud and enumeration target"},
	6443:  {"medium", "orchestration", "Kubernetes API exposed", "kube-apiserver reachable — cluster control plane"},
	8500:  {"medium", "orchestration", "Consul exposed", "Consul HTTP API — service catalog/KV, often unauthenticated"},
	9092:  {"medium", "messaging", "Kafka exposed", "Kafka broker reachable on the network"},
	15672: {"medium", "messaging", "RabbitMQ admin exposed", "RabbitMQ management UI reachable"},
